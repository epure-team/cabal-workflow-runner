...
createServer(async (req,res) => { const url = new URL(req.url ?? "/", `http://${req.headers.host}`); if (req.method !== "GET") return json(res,405,{error:"read-only"}); try {
  ...
  const path = url.pathname === "/" ? "index.html" : url.pathname.slice(1); const target=resolve(publicRoot,path); if(relative(publicRoot,target).startsWith("..")) return json(res,404,{error:"not found"}); res.writeHead(200,{"content-type":mime(target)+"; charset=utf-8","cache-control":"no-store"}); res.end(await readFile(target));
 } catch { json(res,404,{error:"ledger not found or not selected"}); }}).listen(port,"127.0.0.1",()=>console.log(`CWR monitor on 127.0.0.1:${port}; root=${ledgerRoot}`));
